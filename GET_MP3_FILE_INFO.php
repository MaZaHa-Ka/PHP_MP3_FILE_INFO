...
public function getInfo($use_cbr_estimate=false) // true = reread frames and more precisely
{
    $info = array();
    $info["FileSize"] = filesize($this->filename);; // total size
    $info["IdHeaderSize"] = 0; // ID3v2 header size
    $info["AllFramesSize"] = 0; // Total size of all frames
    $info["Duration"] = 0; // cbr true faster? get len from header. no reread frames 
    $info["FramesSizes"] = array(); // array size, count(sizes) == num frames
    $info["NumFrames"] = 0; // Number of frames
    ...
    if ($fd !== false) {
        ...
        $block = fread($fd, 100);
        $offset = $this->skipID3v2Tag($block);
        $info["IdHeaderSize"] = $offset;
        fseek($fd, $offset, SEEK_SET);
        
        $totalFrameSize = 0;
        
        // Read frames to calculate total size and count
        while (!feof($fd))
        {
            $block = fread($fd, 10);
            if (strlen($block) < 10) { break; }
            ...
            {
                $frame_info = self::parseFrameHeader(substr($block, 0, 4));
                if (empty($frame_info['Framesize'])) { return $info; } //some corrupt mp3 files
                //if (!empty($frame_info['Framesize']))
                else
                {
                    $info["AllFramesSize"] += $frame_info['Framesize'];
                    $info["FramesSizes"][] = $frame_info['Framesize'];
                    $info["NumFrames"]++;
                    $info["Duration"] += ( $frame_info['Samples'] / $frame_info['Sampling Rate'] );
                }
                fseek($fd, $frame_info['Framesize'] - 10, SEEK_CUR);
            } else if (substr($block, 0, 3) == 'TAG') {
                fseek($fd, 128 - 10, SEEK_CUR); // Skip over id3v1 tag size
            } else {
                fseek($fd, -9, SEEK_CUR);
            ...
            if ($use_cbr_estimate && !empty($info))
            { 
                $info["Duration"] = $this->estimateDuration($frame_info['Bitrate'],$offset);
            }
        }
        fclose($fd);
    }
...
public function getHeaderSize()
{
   $fd = fopen($this->filename, "rb");
   $block = fread($fd, 100);
   $id3v2TagSize = $this->skipID3v2Tag($block);
   fclose($fd);
   // После определения размера ID3v2 тега, вы можете использовать его,
   // чтобы определить размер заголовка от начала файла до начала звуковых данных
   return $id3v2TagSize;
}
...
<?php
$mp3file = new MP3File("mus\\1.mp3");//http://www.npr.org/rss/podcast.php?id=510282
//$duration1 = $mp3file->getDurationEstimate();//(faster) for CBR only
//$duration2 = $mp3file->getDuration();//(slower) for VBR (or CBR) reread frames
//echo "Duration: $duration1 seconds"."\n<br>";
//$headerSize = $mp3file->getHeaderSize(); // faster
//echo "Header size: $headerSize bytes\n<br>";
//$fileSize = $mp3file->getFileSize();
//echo "File size: $fileSize bytes\n<br>";

$info = $mp3file->getInfo();
echo "File size: ".$info["FileSize"]." bytes\n<br>";
echo "Header size: ".$info["IdHeaderSize"]." bytes\n<br>";
echo "All Frames size: ".$info["AllFramesSize"]." bytes\n<br>";
echo "Duration: ".$info["Duration"]." seconds"."\n<br>";
echo "Frames count: ".$info["NumFrames"]."\n<br>";
echo "Frames count: ".count($info["FramesSizes"])."\n<br>";
?>